...
| T40 | Deliver the human moderation workflow | Moderators and administrators can privately review queued reports, inspect protected media, then publish, request information, reject, or withdraw a published abduction with a mandatory reason, audit entry, and reporter notification. One shared eligibility rule drives the queue, staff badges, and dashboard moderation-decision count; it remains until a real decision resolves the item. | T24, T29, T33, T35 | Completed |
...
| T47 | Build the administrator dashboard | A safe operational dashboard presents moderation volumes and delay, active/pending/expired alerts, hospital and moderator-access requests, moderator activity, and an action-first private work-inbox panel with the oldest pending age. Its moderation action count is identical to the visible moderator queue, not the broader AI-review backlog. | T46 | Complete |
...
| T49 | Define the alert-sheet contract and safety rules | A single English public-safe representation is defined for each alert type: title, category, safe AI summary, approximate coverage, date, status, safety guidance, expiry where applicable, `Source: SAVE-US`, and optional authorised identification-photo metadata. It excludes private contact, precise address/GPS, private circumstances, original media, and road-accident media. | T17, T29, T33, T35 | Completed |
| T50 | Build the printable HTML alert sheet | Published alerts have an authorised A4-friendly English sheet with SAVE-US branding, print styles, generation date, and a working Print action. An explicitly authorised missing-person or abduction identification photo uses only a metadata-free derivative. | T49 | Completed |
| T51 | Generate the server-side PDF alert sheet | The same authorised public-safe sheet can be downloaded as a PDF with a consistent English filename and private, non-cacheable response headers; it may contain the same authorised derivative. | T49 | Completed |
| T52 | Build secure share links | Opaque, revocable and expiring share links expose only approved public-safe content and stop working when their alert is withdrawn, rejected, or expired. An explicitly authorised identification-photo derivative is token-bound; original and road-accident media are absent. | T49 | Completed |
| T53 | Build sharing actions | Alert detail provides copy-link, mobile Web Share fallback, and a prefilled WhatsApp share containing `Source: SAVE-US` and the secure URL. The share page supplies an Open Graph image preview only when an authorised derivative exists. | T50, T51, T52 | Completed |
| T54 | Test alert-sheet and sharing safety | Automated end-to-end tests cover printable HTML, PDF, English content, attribution, WhatsApp payload, link revocation, opt-in metadata-free derivatives, and the absence of private data or unauthorised media. | T50–T53 | Completed |
